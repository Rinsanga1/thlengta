...
async function getDailyAttendanceReport(storeId, date = "now") {
  const sql = `
    SELECT
      e.id,
      e.name,
    ...
    ORDER BY e.name
  `;
  return dbAll(sql, [date, storeId]);
}

async function getLateArrivals(storeId, days = 7) {
  const sql = `
    SELECT
      e.name,
  ...
  `;
  return dbAll(sql, [storeId, days]);
}

async function getEmployeeAttendanceHistory(storeId, startDate, endDate) {
  const sql = `
    SELECT
      DATE(a.created_at) as date,
      e.name,
  ...
  `;
  return dbAll(sql, [storeId, startDate, endDate]);
}

async function getEmployeeCheckInStatus(storeId, date = "now") {
  const sql = `
    SELECT
      e.id,
      e.name,
...
async function getStoreById(storeId) {
  return dbGet("SELECT * FROM stores WHERE id = ?", [storeId]);
}

async function getStoreByPublicId(publicId) {
  return dbGet("SELECT * FROM stores WHERE public_id = ?", [publicId]);
}
...
async function getActiveEmployeesByStore(storeId) {
  return dbAll("SELECT * FROM employees WHERE store_id = ? AND is_active = 1 ORDER BY name", [storeId]);
}
