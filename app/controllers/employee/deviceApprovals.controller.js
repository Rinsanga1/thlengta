...
exports.create = async (req, res) => {
  ...
  try {
    ...
    const employee = await dbGet(
      "SELECT id, email FROM employees WHERE workplace_id = ? AND email = ? AND is_active = 1",
      [workplace.id, employeeEmail]
    );
    ...
    if (!employee) {
      return res.renderPage("employee/device_approvals/new", {
        ...
        clientDeviceToken: String(req.body.client_device_token || ""),
        deniedLogId: String(req.body.deniedLogId || ""),
        error: "Employee not found for this workplace."
      });
    }
    ...
    const userRow = await dbGet(
      ...
      [workplace.user_id, managerEmail]
    );
    ...
    if (!approved) {
      const mgr = await dbGet(
        ...
        [managerEmail, workplace.user_id]
      );
    ...
    if (deniedLogId && /^\d+$/.test(deniedLogId)) {
      ...
      if (logRow) {
        ...
        if (approvedByType === "user") {
          await dbRun("UPDATE attendance_logs SET approved_by_user_id = ? WHERE id = ?", [workplace.user_id, nId]);
    ...
    const timeStatus = computeCheckinTimeStatus(workplace, lat, lng);

    await dbRun(
      `INSERT INTO attendance_logs (
        workplace_id, employee_id, event_type, device_ok, gps_ok,
    ...
    return res.redirect(`/e/scan/${workplacePublicId}?approved=1`);
  } catch (err) {
    console.error(err);
