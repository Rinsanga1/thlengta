...
exports.index = async (req, res) => {
  ...
  res.renderPage("admin/employees/index", { // Renamed view
    title: "Employees",
    store,
    employees,
...
exports.new = async (req, res) => {
  ...
  res.renderPage("admin/employees/new", { // Renamed view
    title: "Add Employee",
    store,
    error: null
  });
...
exports.create = async (req, res) => {
  try {
    ...
    if (!email || !/^\S+@\S+\.\S+$/.test(email)) {
      return res.renderPage("admin/employees/new", {
        title: "Add Employee",
        store,
    ...
    return res.redirect(
      `/admin/stores/${storeId}/employees?msg=` +
...
exports.updateStatus = async (req, res) => {
  try {
    ...
    return res.redirect(`/admin/stores/${storeId}/employees`);
  } catch (err) {
    console.error(err);
    return res.status(500).send("Server error");
  }
...
exports.destroy = async (req, res) => {
  try {
    ...
    return res.redirect(
      `/admin/stores/${storeId}/employees?msg=` + encodeURIComponent(`Deleted employee ${emp.email}.`)
    );
  } catch (err) {
    console.error(err);
    return res.status(500).send("Server error");
