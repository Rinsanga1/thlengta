...
exports.new_step1_post = async (req, res) => {
  ...
  return res.redirect("/admin/stores/new/step-2");
};

// Wizard: Step 2 (Location) - GET
exports.new_step2_get = (req, res) => {
  const draft = ensureWorkplaceDraft(req);
  if (!draft.name) return res.redirect("/admin/stores/new/step-1");

  return res.renderPage("admin/stores/new_step2_location", {
...
exports.new_step2_post = (req, res) => {
  ...
  return res.redirect("/admin/stores/new/step-3");
};

// Wizard: Step 3 (Radius) - GET
exports.new_step3_get = (req, res) => {
  const draft = ensureWorkplaceDraft(req);
  if (!draft.lat || !draft.lng) return res.redirect("/admin/stores/new/step-2");

  if (!draft.radius_m) draft.radius_m = 70;

  return res.renderPage("admin/stores/new_step3_radius", {
...
exports.new_step3_post = (req, res) => {
  ...
  return res.redirect("/admin/stores/new/step-4");
};

// Wizard: Step 4 (Review) - GET
exports.new_step4_get = (req, res) => {
  const draft = ensureWorkplaceDraft(req);
  if (!draft.radius_m) return res.redirect("/admin/stores/new/step-3");

  return res.renderPage("admin/stores/new_step4_review", {
...
exports.create = async (req, res) => {
  try {
    ...
    return res.redirect(`/admin/stores/${storeId}/qr`); // Redirect to new store's QR view
  } catch (err) {
    console.error(err);
    return res.renderPage("admin/stores/new_step4_review", {
      title: "Review Workplace",
...
exports.show = async (req, res) => {
  ...
  res.renderPage("admin/stores/show", { title: "Store QR", store, scanUrl }); // Renamed view
};
...
exports.edit = async (req, res) => {
  ...
  res.renderPage("admin/stores/edit", { // Renamed view
...
exports.update = async (req, res) => {
  try {
    ...
    return res.redirect(`/admin/stores/${storeId}/edit?msg=` + encodeURIComponent("Store times saved."));
  } catch (err) {
    console.error(err);
...
exports.updateLogo = async (req, res) => {
  try {
    ...
    if (!req.file || !req.file.filename) {
      return res.redirect(`/admin/stores/${storeId}/edit?msg=` + encodeURIComponent("No file uploaded."));
    }
    ...
    return res.redirect(`/admin/stores/${storeId}/edit?msg=` + encodeURIComponent("Logo updated."));
  } catch (err) {
    console.error(err);
    return res.status(500).send("Server error");
  }
...
exports.rotateQr = async (req, res) => {
  ...
  res.redirect(`/admin/stores/${storeId}/edit?msg=` + encodeURIComponent("QR rotated. Print the new QR now."));
};
...
exports.testQrDownload = async (req, res) => {
  ...
  return res.renderPage("admin/stores/test_qr_download", {
